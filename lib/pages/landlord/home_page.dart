...
class _LandLordHomePageState extends State<LandLordHomePage> {
  ...
  Future<String> uploadImages(List<XFile> images) async {
    final imageUrls = await Future.wait(images.map((image) async {
      // Use your existing uploadImage function for each image
      return _storageService.uploadImage('houses', image.path);
    }));
    // Combine download URLs into a single string 
    ...
    if (images.isEmpty) {
      ...
      if (typeController.text == "") {
        ...
        if (genderController.text == "") {
          ...
          if (rentTypeController.text == "") {
            errorDialog(context, "Renting Type is required");
            test = false;
          } else {
            if (placesNumberController.text == "" &&
  ...
  MainButton postButton(BuildContext context) {
    return MainButton(
                      ...
                      onPressed: () async {
                        if (checkUserInput()) {
                          try {
                            String url = await uploadImages(images);
                            await _fireStoreService.postHouse(
                                ...
                                typeController.text,
                                placesNumberController.text != ""
                                    ? rentTypeController.text
                                    : "0",
                                currentUser!.uid,
                                genderController.text);
                            successDialog(
                                // ignore: use_build_context_synchronously
                                context, "You're house posted successfully");
                          } catch (e) {
                            print("Errur : $e");
                          }
                        }
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      if (snapshot.hasData) {
                          ...
                          child: Container(
                            ...
                            child: ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return LandLordCard(
                                      path: pathList[0],
                                      city: data[index]["location"].toString(),
                                      gender: data[index]["gender"],
                                      availablePlaces: int.parse(
                                          data[index]["available_places"]),
                                      location:
                                          address == null ? "error" : address!,
                        ...
                        return const Text(
                            "You don't have any posted houses yet");
                      }
                    }
                  }),
            ),
